...
def table_test_fixture():
    ...
    t_manager = table_manager(mod)
    yield t_manager
    for i in test_info:
        t_manager._test_teardown(i)
...
def test_register_table(table_test_fixture, table_info):
    ...
    # *api生成 等待api管理器开发完成
    session = get_session()
    t_manager.register_table(table_info)

    all_table = all_tables()
    # Table Field表更新
    for table in table_info:
        table_row = session.query(Tables).filter_by(name=table['table_name'], owner_id=mod.get('id')).first()
...
def test_delete_table(table_test_fixture, table_info):
    ...
    session = get_session()
    for table in table_info:
        t_manager.delete_table(table['table_name'])
